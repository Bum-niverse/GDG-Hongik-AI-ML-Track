...
또한 이게 분류인지 회귀인지 생각하는 것에서 부터 시작한 과정이었다.
...
이전 주차에서도 배웠듯이, 모델이 이미 본 데이터로만 평가하면 성능이 실제보다 좋게 나올 수 밖에 없다. 
따라서 모델이 새로운 데이터에도 잘 작동하는지 확인하려면 결국 훈련 데이터와 테스트 데이터를 분리해야 한다.
일단 데이터를 각 세트 별로 분리 한뒤, 이후 표준화 과정을 진행해보았다.
...
그래서 지난 강의 때 사용했던 StandardScaler를 사용해 평균과 표준편차를 기준으로 데이터를 변환하였다.
중요한 점은 테스트 세트와 새로운 데이터도 훈련 세트에서 계산한 평균과 표준편차를 기준으로 변환해야 한다는 것이다.
```python
test_data_scaled = ss.transform(test_data)
```
모델이 표준화된 데이터로 학습했기 때문에, 새로 받은 데이터도 표준화 시켜야 값에 오류가 없다.
...
따라서 predict_proba()를 같이 사용하면 모델이 얼마나 확신을 가지고 예측했는지 확률값을 통해 확인할 수 있다.
...
```
SGDClassifier는 이번 강의 때 배운 모델로, 확률적 경사 하강법을 사용하는 분류 모델이다.
...
에포크를 일단 최소치와 최대치로 주고, 모델 비교를 통해서 정확도를 파악한 뒤, 적정 에포크를 찾는 방식으로 진행하였다.
개인적으로는 이 부분이 제일 어려운 것 같다. 에포크의 적정치 범위를 어떻게 설정해야할지 감이 안잡혔다.
...
점수 변화의 적정선을 찾아서, 에포크 값을 찾아내야한다.

## 📌10. 정리

이번 주차에서는 로지스틱 회귀와 SGDClassifier를 사용해서 생선 데이터를 분류해보았다.

로지스틱 회귀는 이름에 회귀가 들어가서 처음에는 회귀 모델처럼 느껴졌지만, 실제로는 분류 문제에도 사용할 수 있었다.  
특히 단순히 예측 결과만 출력하는 것이 아니라, 각 생선 종류에 대한 확률도 함께 확인할 수 있다는 점이 중요했다.

이번 주차에서 정리할 내용은 다음과 같다.

- 로지스틱 회귀는 분류 문제에 사용할 수 있다.
- `predict()`는 최종 예측 결과를 출력한다.
- `predict_proba()`는 각 클래스별 확률을 출력한다.
- 새로운 데이터를 예측할 때도 기존 훈련 데이터와 같은 기준으로 표준화해야 한다.
- SGDClassifier는 데이터를 반복해서 학습하면서 모델을 조금씩 업데이트한다.
- `loss='hinge'`를 사용하면 선형 SVM 방식의 손실 함수를 사용한다.
- 에포크 수가 너무 적으면 학습이 부족할 수 있다. 과소적합이 발생가능하다.
- 반대로 에포크 수가 너무 많으면 훈련 데이터에만 너무 맞춰질 수 있다. 과대적합이 발생가능하다.
- 그래서 훈련 정확도와 테스트 정확도를 그래프로 보고 적절한 에포크 수를 정해야 한다.

결국 이번 주차의 핵심은 모델을 한 번 학습시키고 끝내는 것이 아니라, 예측 확률과 에포크별 정확도 변화를 보면서 모델의 학습 상태를 확인하는 것이었다.
테스트 한두번 한다고 해서 모델이 학습된다고 생각하는건 오산이고, 그렇다고 또 엄청 많이 학습시키는 것도 문제가 되었다.
적정값을 찾아서 적정 결과를 도출해내는 것도 중요하다는 것을 확인했다.

## 📌11. 느낀점

이번 주차에서는 로지스틱 회귀와 SGDClassifier를 사용해서 생선 분류 문제를 풀어보았다.

가장 헷갈렸던 부분은 로지스틱 회귀였다. 이름만 보면 회귀 문제에 사용하는 모델처럼 보이는데, 실제로는 생선 종류를 분류하는 데 사용할 수 있었다. 처음에는 이 부분이 조금 어색했지만, 실습을 하면서 로지스틱 회귀는 각 클래스에 대한 확률을 계산하고, 그중 가장 확률이 높은 클래스를 예측값으로 선택한다는 것을 알게 되었다. 결국 로지스틱 회귀는 분류모델이라는 것을 알 수 있었다.

또 `predict()`와 `predict_proba()`의 차이도 이번에 확실히 정리할 수 있었다. `predict()`는 최종적으로 어떤 생선인지 결과만 보여주고, `predict_proba()`는 각 생선일 확률을 보여준다. 그래서 단순히 정답만 보는 것보다, 모델이 어느 정도 확신을 가지고 예측했는지도 같이 볼 수 있었다.

SGDClassifier에서는 에포크 수를 바꿔가며 정확도가 어떻게 변하는지 확인했다. 처음에는 에포크 수가 많을수록 당연히 더 좋은 모델이 될 것이라고 생각했다. 하지만 그래프를 보면서 꼭 그런 것은 아니라는 것을 알게 되었다. 너무 적게 학습하면 모델이 데이터를 제대로 배우지 못하고, 너무 많이 학습하면 훈련 데이터에만 맞춰질 수 있다. 데이터값만 표준화시키면 당연히 훈련을 적게해도 잘 학습할거라 생각했는데, 그렇지 못했다. 적정선을 찾아서 학습 시키는게 중요하다는 것을 깨달았다.

그래서 이번 과제를 하면서 모델의 정확도만 보는 것이 아니라, 훈련 정확도와 테스트 정확도를 같이 비교해야 한다는 점을 느꼈다. 
앞으로 모델을 학습시킬 때도 단순히 코드가 실행되는지만 확인하지 말고, 왜 이런 결과가 나왔는지 같이 확인해야겠다고 생각했다.
머신러닝 공부를 하면서 가장 많이 체감되는 것은 '왜'인 것 같다.
모델을 만들면서 '왜 이 방식을 사용했지?', '왜 값을 이렇게 둬야할까' 등등 다양한 부분에서 깊게 고민할 수 있게 되었는데, 이런 고민이 모델의 정확도를 늘리는 것에 도움이 된다는 것을 체감하는 중이다.

이번 과제를 통해 모델을 학습시킬 때 단순히 코드를 실행하고 정확도만 확인하는 것이 아니라, 데이터 전처리, 예측 확률, 손실 함수, 에포크 수까지 함께 고려해야 한다는 것을 알게 되었다. 
앞으로 다른 모델을 사용할 때도 점수만 보는 것이 아니라 모델이 어떤 방식으로 학습하고 예측하는지 같이 확인해야겠다고 느꼈다.

최근 여러 사이트에서 올라오는 신규 모델들의 평가 지표를 보고 있는데, 이런 방식으로 평가되는구나~ 하며 배운 것을 적용해서 보고 있는 중이다.
역시 아는 만큼 보이나보다.
